...
impl SceneColor {
    ...
    pub fn expr(emission: Expr<Vec3<f32>>, opacity: Expr<Vec3<f32>>) -> Expr<Self> {
        Self::from_comps_expr(SceneColorComps { emission, opacity })
    }
}
...
impl Scene {
        ...
        Self {
            draws: vec![
                Draw {
                    ...
                    center: Vec2::new(center.x, center.y + height),
                    color: SceneColor::solid(Vec3::splat(0.0)),
                },
                Draw {
                    brush: Brush::Rect(1.0, height),
                    center: Vec2::new(center.x - width, center.y),
    ...
    pub fn sunflower() -> Self {
        ...
        for i in 1..200 {
            ...
            let pos = Vec2::new(center.x + r * angle.cos(), center.y + r * angle.sin());
            ...
            draws.push(Draw {
                ...
                color: SceneColor::new(
                    Vec3::new(
                        color.red.max(0.0) * 10.0,
                        color.green.max(0.0) * 10.0,
                        color.blue.max(0.0) * 10.0,
                    ),
                    Vec3::splat(0.7),
                ),
            });
